...
const (
	...
	DefaultJwtSecret="this is default secret key"
	UserIdKey="user_id"
	RoleKey="role"
)
...
const (
	...
	QueryUpdateUser             = "UPDATE users SET first_name=?, last_name=?, role=?, state=? WHERE id=?;"
	QueryDeleteUser             = "UPDATE users SET state=?, role=? WHERE id=?;"
